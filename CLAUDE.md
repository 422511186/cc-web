...
CodeRelay 把本地 Claude Code 的聊天搬上 Web：既能浏览历史聊天记录，也能在网页（含手机）里**接管并续聊**同一个会话。设计文档见 `docs/superpowers/specs/`（`2026-06-14-cc-web-design.md` 历史浏览、`2026-06-14-cc-web-realtime-conversation-design.md` 实时续聊）。
...
# 构建全部包（shared 必须先构建，host/web 依赖它的 dist 类型）
npm run build
...
npm run dev:web      # 启动 CodeRelay Web Vite dev server（端口 3000，/api 代理到 3002）
...
当前 workspace 分为运行时应用与共享库/未来基础包，核心动机是**Host 与 Web 共享 TypeScript 类型**，避免 API 契约漂移：
...
TS 项目引用（project references）：host/web 的 `tsconfig.json` 都 `references` shared，根 `tsconfig.base.json` 为共享基础配置（`composite`、`strict`、ES2022）。
...
- 改动 shared 类型后记得 `npm run build`（或至少构建 shared），否则 host/web 拿到的是旧类型。

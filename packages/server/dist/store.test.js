...
describe('SessionStore', () => {
    ...
    describe('listSessions', () => {
        it('should return sessions from JSONL files in project directory', async () => {
            ...
            vi.mocked(fs.readFile).mockImplementation(async (p) => {
                const pathStr = p.toString();
                if (pathStr.includes('session1')) {
                    return '{"type":"user","text":"Hello"}\n{"type":"assistant","text":"Hi"}';
                }
                if (pathStr.includes('session2')) {
                    return '{"type":"user","text":"Question"}';
                }
                return '';
            });
        ...
        it('should handle project with no JSONL files', async () => {
            vi.mocked(fs.readdir).mockResolvedValue(['readme.md']);
            vi.mocked(fs.stat).mockResolvedValue({ isFile: () => false });
    ...
    describe('getSession', () => {
        it('should return session detail with all messages', async () => {
            ...
            vi.mocked(fs.readFile).mockResolvedValue('{"type":"user","text":"Hello","timestamp":1000}\n{"type":"assistant","text":"Hi","timestamp":2000}');

...
describe("coverage configuration", () => {
  it("root 与各 workspace 都应提供 test:coverage 脚本", () => {
    ...
    expect(rootPkg.scripts.build).toBe(
      "npm run build --workspace @coderelay/shared && npm run build --workspace @coderelay/transport && npm run build --workspace @coderelay/p2p-core && npm run build --workspaces --if-present"
    );

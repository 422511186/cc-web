...
会话维护 `mode`，而不是每个浏览器本地维护。当前模式集合参考 Claude Code / VSCode 插件：

- `default` / Ask before edits：危险操作与文件编辑前请求确认。
- `acceptEdits` / Edit automatically：自动接受文件编辑，但危险命令仍走权限链路。
- `plan` / Plan mode：只做探索和计划，通过 ExitPlanMode 提交计划审批。
- `auto`：默认推荐模式，由 Host 根据当前场景映射到安全的 SDK 配置。
- `bypassPermissions` / Bypass permissions：高风险模式。Host 启动 SDK query 时始终带 `allowDangerouslySkipPermissions` 能力位，以便运行中可以切入该模式；实际权限行为仍由当前 `permissionMode` 控制。

模式变化通过 `mode_changed` 广播给所有订阅者。后续消息默认使用当前会话模式。

实现上，Host 在启动 SDK query 时传入当前 `mode`，并在 Web 调用 `PATCH /sessions/:runId/mode` 时通过 SDK streaming control 的 `setPermissionMode()` 同步给正在运行的 query。接口仍返回 `appliesTo: "next_turn"`，表示 UI 不强行中断当前回合，新模式从后续模型决策开始生效。

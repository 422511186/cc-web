...
- ✅ 恢复前会先调用 `GET /api/sessions/:runId` 做快速探活；若 run 已失效，则立即清理脏映射并回到“在此继续”
- ✅ 切走再切回同一历史会话时：
  - 忙碌 run（`executing` / `waiting`）→ 自动接管
  - 空闲但仍活着的 run（`idle`）→ 也自动接管
  - 只有 run 实际已结束 / 已失效时，才清理 `activeRuns` 并重新显示“在此继续”
...
**影响**: 切换后再切回更快，因为后端 run 没有被悄悄 release 掉；也为活跃 agent 列表提供了稳定的生命周期基础。

---

### 14. ActiveRuns 从“续聊辅助索引”升级为“活跃 agent 管理配角”
...
- ✅ 新增 `POST /api/sessions/:runId/close` 作为活跃 agent 的显式关闭接口

**影响**: 活跃 agent 列表现在既能包含“历史续聊 run”，也能包含“纯新建 run”，前端切换与关闭动作都更一致。

---

### 15. 快捷新建从“只会 prompt 输入路径”演进到“复用已有项目目录”

**设计文档**: 早期只有“新建对话”概念，没有明确区分“自定义目录新建”和“从已有项目直接新建”。
...
- ✅ 后端超限时统一返回 `409`
- ✅ 前端 Sidebar 与状态栏都开始显示“已达上限，请先关闭一个 agent”
...
| 2026-06-18 | 同步活跃 agent 列表、项目级快速新建、切换只切视图、默认上限改为 3 |
